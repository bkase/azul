...
const ui = {
  ...
  scores: document.getElementById("scores-text"),
  board: document.getElementById("board-text"),
  turnPill: document.getElementById("turn-pill"),
  checkpointUrl: document.getElementById("checkpoint-url"),
  reloadModel: document.getElementById("reload-model"),
  newGame: document.getElementById("new-game"),
  aiFirst: document.getElementById("ai-first"),
  mctsSims: document.getElementById("mcts-sims"),
  actionSelect: document.getElementById("action-select"),
  playMove: document.getElementById("play-move"),
  randomMove: document.getElementById("random-move"),
...
async function readArray(array) {
  if (array == null) {
...
function parseSafetensors(buffer) {
  ...
  for (const [name, info] of Object.entries(header)) {
    ...
    if (info.dtype === "F32") {
      ...
      throw new Error(`Unsupported dtype ${info.dtype} for tensor ${name}`);
    }
    tensors[name] = {
      data,
      shape: info.shape,
...
async function maybeRunAi() {
  while (!game.is_game_over() && game.current_player() !== humanPlayer) {
    ...
    const sims = Number.parseInt(ui.mctsSims.value, 10) || 32;
